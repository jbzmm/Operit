...
function getNativeInterface() {
    const candidate = globalThis === null || globalThis === void 0 ? void 0 : globalThis.NativeInterface;
...
function sqlTransaction(operations) {
    const nativeInterface = getNativeInterface();
...
function execOrThrow(sql, params = []) {
...
function ensureSchemaReady() {
    ...
    try {
        ...
        const now = nowMs();
        execOrThrow("INSERT OR REPLACE INTO memory_metadata(profile_id, key, value, updated_at) VALUES (?, ?, ?, ?)", [profileId, "schema_version", SCHEMA_VERSION, now]);
...
function listFolders(profileId) {
    const rows = queryRows("SELECT DISTINCT folder_path FROM memory_nodes WHERE profile_id = ? AND deleted_at IS NULL AND folder_path IS NOT NULL AND TRIM(folder_path) <> '' ORDER BY folder_path ASC", [profileId]);
    return rows
...
function normalizeChunksInput(rawChunks) {
...
function loadNodeById(profileId, id) {
    const row = queryOne("SELECT * FROM memory_nodes WHERE profile_id = ? AND id = ? AND deleted_at IS NULL LIMIT 1", [profileId, id]);
    return normalizeNodeRow(row);
}
function listChunksForNode(profileId, nodeId) {
    const rows = queryRows("SELECT id, node_id, chunk_index, content, token_count, created_at, updated_at FROM memory_chunks WHERE profile_id = ? AND node_id = ? ORDER BY chunk_index ASC", [profileId, nodeId]);
    return rows.map(row => ({
        id: asText(row.id, ""),
...
function collectEdgeMap(profileId, nodeIds) {
    ...
    const rows = queryRows(`SELECT source_node_id, target_node_id, weight FROM memory_links WHERE profile_id = ? AND (source_node_id IN (${placeholders}) OR target_node_id IN (${placeholders}))`, [profileId].concat(nodeIds).concat(nodeIds));
...
async function searchNodes(profileId, queryText, folderPath, limit, settings, cloudConfig) {
    ...
    const rows = queryRows(`SELECT * FROM memory_nodes WHERE profile_id = ? AND deleted_at IS NULL AND is_placeholder = 0${folderFilter.clause} ORDER BY updated_at DESC LIMIT 500`, [profileId].concat(folderFilter.params));
    ...
    if (queryEmbedding && queryEmbedding.length > 0) {
        ...
        if (nodeIds.length > 0) {
            const placeholders = nodeIds.map(() => "?").join(", ");
            const embeddings = queryRows(`SELECT target_id, vector_json FROM memory_embeddings WHERE profile_id = ? AND target_type = 'node' AND provider = ? AND model = ? AND target_id IN (${placeholders})`, [profileId, cloudConfig.provider, cloudConfig.model].concat(nodeIds));
    ...
    const scored = nodes.map((node, index) => {
        ...
        const finalScore = (keywordScore * searchSettings.keywordWeight * mode.keyword) +
    ...
    const filtered = scored
        ...
        .map(item => (Object.assign(Object.assign({}, item.node), { score: {
...
function listLinksForNodes(profileId, nodeIds) {
    ...
    const placeholders = nodeIds.map(() => "?").join(", ");
    const rows = queryRows(`SELECT * FROM memory_links WHERE profile_id = ? AND (source_node_id IN (${placeholders}) OR target_node_id IN (${placeholders})) ORDER BY updated_at DESC LIMIT 500`, [profileId].concat(nodeIds).concat(nodeIds));
...
function getLatestRebuildJob(profileId) {
    const row = queryOne("SELECT * FROM memory_rebuild_jobs WHERE profile_id = ? ORDER BY updated_at DESC LIMIT 1", [profileId]);
...
function buildDimensionStats(profileId) {
    const memoryTotal = queryCount("SELECT COUNT(1) AS c FROM memory_nodes WHERE profile_id = ? AND deleted_at IS NULL AND is_placeholder = 0", [profileId]);
    const chunkTotal = queryCount("SELECT COUNT(1) AS c FROM memory_chunks WHERE profile_id = ?", [profileId]);
    const memoryEmbedded = queryCount("SELECT COUNT(DISTINCT target_id) AS c FROM memory_embeddings WHERE profile_id = ? AND target_type = 'node'", [profileId]);
    const chunkEmbedded = queryCount("SELECT COUNT(DISTINCT target_id) AS c FROM memory_embeddings WHERE profile_id = ? AND target_type = 'chunk'", [profileId]);
    const memoryDimensions = queryRows("SELECT dimension, COUNT(1) AS count FROM memory_embeddings WHERE profile_id = ? AND target_type = 'node' GROUP BY dimension ORDER BY count DESC, dimension ASC", [profileId]).map(item => ({
        dimension: asInt(item.dimension, 0),
        count: asInt(item.count, 0)
    }));
    const chunkDimensions = queryRows("SELECT dimension, COUNT(1) AS count FROM memory_embeddings WHERE profile_id = ? AND target_type = 'chunk' GROUP BY dimension ORDER BY count DESC, dimension ASC", [profileId]).map(item => ({
        dimension: asInt(item.dimension, 0),
        count: asInt(item.count, 0)
    }));
    const grouped = queryRows("SELECT provider, model, target_type, dimension, COUNT(1) AS count FROM memory_embeddings WHERE profile_id = ? GROUP BY provider, model, target_type, dimension ORDER BY count DESC", [profileId]).map(item => ({
...
function buildDashboard(profileId, queryText, folderPath, limit, settings, cloudConfig, dimensionStats, rebuildJob) {
    return searchNodes(profileId, queryText, folderPath, limit, settings, cloudConfig).then(nodes => {
        ...
        return {
            ...
            rebuildJob,
            stats: {
                nodeCount: queryCount("SELECT COUNT(1) AS c FROM memory_nodes WHERE profile_id = ? AND deleted_at IS NULL AND is_placeholder = 0", [profileId]),
...
function upsertEmbedding(profileId, targetType, targetId, vector, cloudConfig) {
    ...
    execOrThrow("INSERT OR REPLACE INTO memory_embeddings(id, profile_id, target_type, target_id, provider, model, dimension, vector_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [embeddingId, profileId, targetType, targetId, provider, model, vector.length, JSON.stringify(vector), now, now]);
}
...
function queryRebuildJob(profileId, jobId) {
    const row = queryOne("SELECT * FROM memory_rebuild_jobs WHERE profile_id = ? AND job_id = ? LIMIT 1", [profileId, jobId]);
    return row ? mapRebuildJob(row) : null;
}
function queryRunningRebuildJob(profileId) {
    const row = queryOne("SELECT * FROM memory_rebuild_jobs WHERE profile_id = ? AND status = 'running' ORDER BY updated_at DESC LIMIT 1", [profileId]);
    return row ? mapRebuildJob(row) : null;
}
function loadNodesBatch(profileId, offset, limit) {
    return queryRows("SELECT * FROM memory_nodes WHERE profile_id = ? AND deleted_at IS NULL AND is_placeholder = 0 ORDER BY id ASC LIMIT ? OFFSET ?", [profileId, limit, offset]).map(normalizeNodeRow).filter(isNonNull);
}
function loadChunksBatch(profileId, offset, limit) {
    return queryRows("SELECT id, node_id, chunk_index, content, token_count, created_at, updated_at FROM memory_chunks WHERE profile_id = ? ORDER BY id ASC LIMIT ? OFFSET ?", [profileId, limit, offset]).map(row => ({
        id: asText(row.id, ""),
        nodeId: asText(row.node_id, ""),
        chunkIndex: asInt(row.chunk_index, 0),
        content: asText(row.content, ""),
...
async function runRebuildJobSlice(profileId, jobId, requestedBatchSize) {
    ...
    if (!cloudConfig.ready) {
        const now = nowMs();
        execOrThrow("UPDATE memory_rebuild_jobs SET status = 'failed', error_text = ?, updated_at = ?, finished_at = ? WHERE profile_id = ? AND job_id = ?", ["cloud embedding config invalid", now, now, profileId, jobId]);
        return queryRebuildJob(profileId, jobId);
    }
    const nodeTotal = queryCount("SELECT COUNT(1) AS c FROM memory_nodes WHERE profile_id = ? AND deleted_at IS NULL AND is_placeholder = 0", [profileId]);
    const chunkTotal = queryCount("SELECT COUNT(1) AS c FROM memory_chunks WHERE profile_id = ?", [profileId]);
    ...
    execOrThrow("UPDATE memory_rebuild_jobs SET status = ?, current_stage = ?, total_count = ?, processed_count = ?, failed_count = ?, cursor_index = ?, updated_at = ?, finished_at = ? WHERE profile_id = ? AND job_id = ?", [status, finalStage, total, processed, failed, cursor, now, finished ? now : null, profileId, jobId]);
    return queryRebuildJob(profileId, jobId);
}
function deleteNodesByIds(profileId, ids) {
    ...
    const chunkRows = queryRows(`SELECT id FROM memory_chunks WHERE profile_id = ? AND node_id IN (${placeholders})`, [profileId].concat(uniqueIds));
...
async function memory_start_rebuild_embedding_job(params) {
    ...
    execOrThrow("INSERT INTO memory_rebuild_jobs(job_id, profile_id, status, current_stage, total_count, processed_count, failed_count, cursor_index, started_at, updated_at, finished_at, config_json, error_text) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [jobId, profileId, "running", "preparing", totalCount, 0, 0, 0, now, now, null, JSON.stringify(config), ""]);
...
async function memory_upsert_node(params) {
    ...
    execOrThrow("INSERT OR REPLACE INTO memory_nodes(id, profile_id, node_type, title, content, source, content_type, folder_path, is_document_node, is_placeholder, importance, credibility, tags_json, extra_json, created_at, updated_at, deleted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)", [
        id,
        profileId,
        nodeType,
        title,
    ...
    if (chunks.length > 0) {
        ...
        for (const chunk of chunks) {
            ...
            execOrThrow("INSERT INTO memory_chunks(id, profile_id, node_id, chunk_index, content, token_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [chunkId, profileId, id, chunk.chunkIndex, chunk.content, Math.max(0, asInt(chunk.content.length / 4, 0)), now, now]);
        }
    }
    const cloud = loadCloudConfig();
    if (cloud.ready) {
...
async function memory_move_nodes_to_folder(params) {
    ...
    const placeholders = uniqueIds.map(() => "?").join(", ");
    const updateResult = sqlExecute(`UPDATE memory_nodes SET folder_path = ?, updated_at = ? WHERE profile_id = ? AND id IN (${placeholders})`, [targetFolder || null, nowMs(), profileId].concat(uniqueIds));
...
async function memory_create_folder(params) {
    ...
    const exists = queryCount("SELECT COUNT(1) AS c FROM memory_nodes WHERE profile_id = ? AND deleted_at IS NULL AND folder_path = ?", [profileId, folderPath]);
    if (exists <= 0) {
        const now = nowMs();
        const placeholderId = createId("folder");
        execOrThrow("INSERT INTO memory_nodes(id, profile_id, node_type, title, content, source, content_type, folder_path, is_document_node, is_placeholder, importance, credibility, tags_json, extra_json, created_at, updated_at, deleted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)", [
            placeholderId,
            profileId,
            "folder_placeholder",
            ".folder_placeholder",
...
async function memory_rename_folder(params) {
    ...
    const rows = queryRows("SELECT id, folder_path FROM memory_nodes WHERE profile_id = ? AND deleted_at IS NULL AND (folder_path = ? OR folder_path LIKE ?)", [profileId, oldPath, `${oldPath}/%`]);
    const now = nowMs();
    for (const row of rows) {
        ...
        execOrThrow("UPDATE memory_nodes SET folder_path = ?, updated_at = ? WHERE profile_id = ? AND id = ?", [replaced, now, profileId, id]);
    }
    return {
        success: true,
        profileId,
...
async function memory_delete_folder(params) {
    ...
    const rows = queryRows("SELECT id FROM memory_nodes WHERE profile_id = ? AND deleted_at IS NULL AND (folder_path = ? OR folder_path LIKE ?)", [profileId, folderPath, `${folderPath}/%`]);
...
async function memory_upsert_link(params) {
    ...
    const now = nowMs();
    const existing = queryOne("SELECT created_at FROM memory_links WHERE profile_id = ? AND id = ? LIMIT 1", [profileId, id]);
    const createdAt = existing ? asInt(existing.created_at, now) : now;
    execOrThrow("INSERT OR REPLACE INTO memory_links(id, profile_id, source_node_id, target_node_id, link_type, weight, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", [
        id,
        profileId,
        sourceNodeId,
        targetNodeId,
