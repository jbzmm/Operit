...
exports.buildMemoryCenterScreen = buildMemoryCenterScreen;
const service_1 = require("./service");
const runtime_tool_1 = require("../shared/runtime_tool");
const format_1 = require("../shared/format");
const service_2 = require("../memory_settings/service");
...
function splitDocumentChunks(content) {
    return String(content || "")
        .split(/\n\s*\n/g)
        .map(item => item.trim())
        .filter(Boolean);
}
function buildDocumentContent(chunks) {
    return chunks
...
function compactDimensionList(items) {
...
function normalizeGraphLayout(value) {
    ...
    const result = {};
    for (const [rawId, rawPoint] of Object.entries(value)) {
...
function ensureGraphLayout(nodeIds, currentLayout) {
...
function buildMemoryCenterScreen(ctx) {
    ...
    const dashboardState = useValueState(ctx, "mc_dashboard", EMPTY_DASHBOARD);
    const boxModeState = useValueState(ctx, "mc_box_mode", false);
    const linkModeState = useValueState(ctx, "mc_link_mode", false);
    const selectedIdsState = useValueState(ctx, "mc_selected_ids", []);
    const graphLayoutState = useValueState(ctx, "mc_graph_layout", {});
    ...
    const documentChunksState = useValueState(ctx, "mc_document_chunks", []);
    ...
    const semanticThresholdState = useValueState(ctx, "mc_settings_semantic_threshold", String(DEFAULT_SETTINGS_PAYLOAD.search.semanticThreshold));
    const scoreModeState = useValueState(ctx, "mc_settings_score_mode", DEFAULT_SETTINGS_PAYLOAD.search.scoreMode);
    const keywordWeightState = useValueState(ctx, "mc_settings_keyword_weight", String(DEFAULT_SETTINGS_PAYLOAD.search.keywordWeight));
    const vectorWeightState = useValueState(ctx, "mc_settings_vector_weight", String(DEFAULT_SETTINGS_PAYLOAD.search.vectorWeight));
    const edgeWeightSettingState = useValueState(ctx, "mc_settings_edge_weight", String(DEFAULT_SETTINGS_PAYLOAD.search.edgeWeight));
    ...
    const dimensionUsageState = useValueState(ctx, "mc_settings_dimension_usage", DEFAULT_DIMENSION_USAGE);
    const rebuildJobState = useValueState(ctx, "mc_settings_rebuild_job", null);
    ...
    const applyLoadedSettings = (payload) => {
        const search = (payload === null || payload === void 0 ? void 0 : payload.search) || DEFAULT_SETTINGS_PAYLOAD.search;
        const cloud = (payload === null || payload === void 0 ? void 0 : payload.cloud) || DEFAULT_SETTINGS_PAYLOAD.cloud;
    ...
    const collectSettingsPayload = () => ({
        search: {
            semanticThreshold: (0, format_1.clampNumber)(semanticThresholdState.value, 0, 1, DEFAULT_SETTINGS_PAYLOAD.search.semanticThreshold),
            scoreMode: String(scoreModeState.value || "balanced").trim() || "balanced",
            keywordWeight: (0, format_1.clampNumber)(keywordWeightState.value, 0, 1000, DEFAULT_SETTINGS_PAYLOAD.search.keywordWeight),
            vectorWeight: (0, format_1.clampNumber)(vectorWeightState.value, 0, 1000, DEFAULT_SETTINGS_PAYLOAD.search.vectorWeight),
            edgeWeight: (0, format_1.clampNumber)(edgeWeightSettingState.value, 0, 1000, DEFAULT_SETTINGS_PAYLOAD.search.edgeWeight)
        },
        cloud: {
    ...
    const refreshSettingsData = async () => {
        ...
        try {
            const payload = await (0, service_2.loadSettings)(ctx);
            const usage = await (0, service_2.loadDimensionUsage)(ctx);
    ...
    const saveSettingsAction = async () => {
        ...
        try {
            ...
            const saved = await (0, service_2.saveSettings)(ctx, payload);
    ...
    const startRebuildAction = async () => {
        setSettingsLoading(true);
        try {
            const job = await (0, service_2.startRebuild)(ctx, 20);
    ...
    const refreshRebuildProgressAction = async () => {
        ...
        setSettingsLoading(true);
        try {
            const next = await (0, service_2.getRebuildProgress)(ctx, job.jobId, 20);
    ...
    const selectedIds = asArray(selectedIdsState.value)
        .map(item => normalizeId(item))
        .filter(Boolean);
    const setSelectedIds = (values) => {
    ...
    const refreshDashboard = async () => {
        setLoading(true);
        try {
            const dashboard = await (0, service_1.loadDashboard)(ctx, queryState.value, selectedFolderState.value, 180);
            dashboardState.set(dashboard);
            graphLayoutState.set(ensureGraphLayout(asArray(dashboard.nodes).map(item => normalizeId(item.id)), normalizeGraphLayout(graphLayoutState.value)));
            clearError();
        }
        catch (error) {
            setError((0, runtime_tool_1.asErrorText)(error));
    ...
    const bootstrap = async () => {
        setLoading(true);
        try {
            const dashboard = await (0, service_1.bootstrapDashboard)(ctx, queryState.value, selectedFolderState.value, 180);
            dashboardState.set(dashboard);
            graphLayoutState.set(ensureGraphLayout(asArray(dashboard.nodes).map(item => normalizeId(item.id)), normalizeGraphLayout(graphLayoutState.value)));
            clearError();
        }
        ...
        try {
            const chunks = editorIsDocumentState.value
                ? editorContentState.value
                    ...
                    .filter(Boolean)
                : [];
            await (0, service_1.upsertNode)(ctx, {
    ...
    const importDocumentText = async () => {
    ...
    const updateDocumentChunk = (chunkIndex, value) => {
        const chunks = asArray(documentChunksState.value).map(item => String(item == null ? "" : item));
    ...
    const addDocumentChunk = () => {
        const chunks = asArray(documentChunksState.value).map(item => String(item == null ? "" : item));
        documentChunksState.set(chunks.concat([""]));
    };
    const removeDocumentChunk = (chunkIndex) => {
        const chunks = asArray(documentChunksState.value).map(item => String(item == null ? "" : item));
        if (chunkIndex < 0 || chunkIndex >= chunks.length) {
            return;
        }
    ...
    const saveDocumentView = async () => {
        ...
        const chunks = asArray(documentChunksState.value)
    ...
    const removeOneNode = async (id) => {
        setLoading(true);
        try {
            await (0, service_1.deleteNode)(ctx, id);
    ...
    const removeSelectedNodes = async () => {
        ...
        setLoading(true);
        try {
            await (0, service_1.bulkDeleteNodes)(ctx, selectedIds);
            setSelectedIds([]);
    ...
    const moveSelected = async () => {
        ...
        setLoading(true);
        try {
            await (0, service_1.moveNodesToFolder)(ctx, selectedIds, targetFolder);
            setSelectedIds([]);
    ...
    const createFolderAction = async () => {
        ...
        setLoading(true);
        try {
            await (0, service_1.createFolder)(ctx, folderPath);
    ...
    const renameFolderAction = async () => {
        ...
        setLoading(true);
        try {
            await (0, service_1.renameFolder)(ctx, oldPath, newPath);
    ...
    const deleteFolderAction = async () => {
        ...
        setLoading(true);
        try {
            await (0, service_1.deleteFolder)(ctx, folderPath);
    ...
    const submitLink = async () => {
        ...
        setLoading(true);
        try {
            await (0, service_1.upsertLink)(ctx, {
    ...
    const removeLink = async (id) => {
        setLoading(true);
        try {
            await (0, service_1.deleteLink)(ctx, id);
    ...
    const openEdgeDetail = (link, editMode = false) => {
    ...
    const handleGraphNodeClick = (payload) => {
        ...
        const target = asArray(dashboardState.value.nodes).find(item => normalizeId(item.id) === nodeId);
        if (target) {
            selectedNodeIdState.set(nodeId);
            memoryInfoOpenState.set(true);
        }
    };
    const handleGraphEdgeClick = (payload) => {
        const map = payload && typeof payload === "object" ? payload : {};
        const edgeId = normalizeId(map.id);
        if (!edgeId) {
            return;
        }
        const target = asArray(dashboardState.value.links).find(item => normalizeId(item.id) === edgeId);
        if (target) {
            openEdgeDetail(target, false);
        }
    };
    const handleGraphNodeDragEnd = (payload) => {
        var _a, _b, _c, _d;
        const map = payload && typeof payload === "object" ? payload : {};
        const nodeId = normalizeId(map.id);
        if (!nodeId) {
        ...
        normalized[nodeId] = {
            x: asFiniteNumber(map.x, (_b = (_a = normalized[nodeId]) === null || _a === void 0 ? void 0 : _a.x) !== null && _b !== void 0 ? _b : 120),
            y: asFiniteNumber(map.y, (_d = (_c = normalized[nodeId]) === null || _c === void 0 ? void 0 : _c.y) !== null && _d !== void 0 ? _d : 120)
        };
        graphLayoutState.set(normalized);
    };
    const allNodes = asArray(dashboardState.value.nodes);
    const allLinks = asArray(dashboardState.value.links);
    const allFolders = asArray(dashboardState.value.folders);
    const graphLayout = ensureGraphLayout(allNodes.map(item => normalizeId(item.id)), normalizeGraphLayout(graphLayoutState.value));
    ...
    const documentChunks = asArray(documentChunksState.value).map(item => String(item == null ? "" : item));
    ...
    const folderPanelNode = ctx.Card({ width: 340, fillMaxSize: true }, [
        ctx.Column({ padding: 12, spacing: 8 }, [
            ...
            ctx.LazyColumn({ spacing: 6, height: 220 }, [
                ...
                ...allFolders.map(folder => ctx.Button({
    ...
    if (boxModeState.value) {
        actionChildren.push(ctx.Button({
            text: `删 ${selectedIds.length}`,
            enabled: selectedIds.length > 0,
            onClick: () => batchDeleteConfirmOpenState.set(true)
        }));
    }
    actionChildren.push(ctx.Button({ text: boxModeState.value ? "框选关" : "框选开", onClick: toggleBoxMode }), ctx.Button({ text: linkModeState.value ? "连线关" : "连线开", onClick: toggleLinkMode }), ctx.Button({ text: "导入", onClick: () => importDialogOpenState.set(true) }), ctx.Button({ text: "新建", onClick: () => openEditor() }), ctx.Button({ text: "刷新", onClick: refreshDashboard }));
    rootChildren.push(ctx.Row({ fillMaxSize: true, spacing: 10, verticalAlignment: "start" }, [
    ...
    if (errorState.value.trim()) {
        rootChildren.push(ctx.Card({ fillMaxWidth: true, containerColor: "errorContainer", contentColor: "onErrorContainer" }, [
    ...
    if (batchDeleteConfirmOpenState.value) {
        rootChildren.push(ctx.Card({ fillMaxWidth: true, containerColor: "tertiaryContainer", contentColor: "onTertiaryContainer" }, [
            ctx.Column({ padding: 12, spacing: 8 }, [
    ...
    if (memoryInfoOpenState.value && selectedMemory) {
        rootChildren.push(ctx.Card({ fillMaxWidth: true, containerColor: "secondaryContainer", contentColor: "onSecondaryContainer" }, [
            ctx.Column({ padding: 12, spacing: 8 }, [
                ctx.Row({ fillMaxWidth: true, verticalAlignment: "center" }, [
                ...
                ctx.Text({
                    text: (0, format_1.compactText)(selectedMemory.content, 360) || "(空内容)",
                    style: "bodySmall",
                    color: "onSurfaceVariant"
                }),
                ctx.Text({
                    text: `文件夹: ${selectedMemory.folderPath || "未分类"} · 类型: ${selectedMemory.nodeType || "text"} · 更新时间: ${(0, format_1.formatDateTime)(selectedMemory.updatedAt)}`,
                    style: "bodySmall",
                    color: "onSurfaceVariant"
                }),
                ctx.Row({ spacing: 8 }, [
    ...
    if (linkDialogOpenState.value && linkSourceState.value && linkTargetState.value) {
        ...
        rootChildren.push(ctx.Card({ fillMaxWidth: true, containerColor: "secondaryContainer", contentColor: "onSecondaryContainer" }, [
            ctx.Column({ padding: 12, spacing: 8 }, [
                ...
                ctx.Text({ text: `${(source === null || source === void 0 ? void 0 : source.title) || linkSourceState.value} -> ${(target === null || target === void 0 ? void 0 : target.title) || linkTargetState.value}`, style: "bodyMedium" }),
                ctx.Row({ spacing: 8, fillMaxWidth: true }, [
                    ...
                    ctx.TextField({
                        label: "权重",
                        value: (0, format_1.toNumberInput)(linkWeightState.value, "1"),
            ...
            ctx.Column({ padding: 12, spacing: 8 }, [
                ...
                rebuildJob
                    ? ctx.Column({ spacing: 4 }, [
                        ...
                        ctx.Text({
                            text: `${(0, format_1.formatPercent)(rebuildJob.fraction || 0)} · ${rebuildJob.status} · ${rebuildJob.currentStage}`,
                            style: "bodySmall",
                            color: "onSurfaceVariant"
                        })
                    ])
        ...
        rootChildren.push(ctx.Card({ fillMaxWidth: true, containerColor: "secondaryContainer", contentColor: "onSecondaryContainer" }, [
            ctx.Column({ padding: 12, spacing: 8 }, [
                ...
                ctx.LazyColumn({ spacing: 8, fillMaxWidth: true, height: 280 }, [
                    ...visibleDocumentChunkIndexes.map(chunkIndex => ctx.Card({ fillMaxWidth: true }, [
            ...
            ctx.Column({ padding: 12, spacing: 8 }, [
                ...
                edgeEditModeState.value
                    ? ctx.Column({ spacing: 8 }, [
                        ...
                        ctx.TextField({
                            label: "权重",
                            value: (0, format_1.toNumberInput)(edgeWeightState.value, "1"),
