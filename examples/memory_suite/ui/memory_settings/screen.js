...
exports.buildMemorySettingsScreen = buildMemorySettingsScreen;
...
function applySearchDefaults(thresholdState, scoreModeState, keywordState, vectorState, edgeState) {
...
function buildMemorySettingsScreen(ctx) {
    ...
    const dimensionUsageState = useValueState(ctx, "ms_dimension_usage", DEFAULT_DIMENSION_USAGE);
    const rebuildJobState = useValueState(ctx, "ms_rebuild_job", null);
    ...
    const applyLoadedSettings = (search, cloud) => {
    ...
    const collectSettings = () => {
        return {
            search: {
                semanticThreshold: (0, format_1.clampNumber)(semanticThresholdState.value, 0, 1, DEFAULT_SEARCH.semanticThreshold),
                scoreMode: String(scoreModeState.value || "balanced").trim() || "balanced",
                keywordWeight: (0, format_1.clampNumber)(keywordWeightState.value, 0, 1000, DEFAULT_SEARCH.keywordWeight),
                vectorWeight: (0, format_1.clampNumber)(vectorWeightState.value, 0, 1000, DEFAULT_SEARCH.vectorWeight),
                edgeWeight: (0, format_1.clampNumber)(edgeWeightState.value, 0, 1000, DEFAULT_SEARCH.edgeWeight)
            },
            cloud: {
    ...
    const refreshAll = async () => {
        ...
        try {
            const settings = await (0, service_1.loadSettings)(ctx);
            const usage = await (0, service_1.loadDimensionUsage)(ctx);
    ...
    const saveAction = async () => {
        ...
        try {
            ...
            const saved = await (0, service_1.saveSettings)(ctx, next);
    ...
    const startRebuildAction = async () => {
        loadingState.set(true);
        try {
            const job = await (0, service_1.startRebuild)(ctx, 20);
    ...
    const refreshRebuildProgressAction = async () => {
        ...
        loadingState.set(true);
        try {
            const next = await (0, service_1.getRebuildProgress)(ctx, current.jobId, 20);
        ...
        ctx.Card({ fillMaxWidth: true }, [
            ctx.Column({ padding: 12, spacing: 8 }, [
                ...
                ctx.Row({ spacing: 8 }, [
                    ...
                    ctx.Button({
                        text: "重置默认",
                        onClick: () => applySearchDefaults(semanticThresholdState, scoreModeState, keywordWeightState, vectorWeightState, edgeWeightState)
                    })
                ])
            ])
        ]),
            ...
            ctx.Column({ padding: 12, spacing: 8 }, [
                ...
                ctx.Text({
                    text: usage.grouped.length > 0
                        ? (0, format_1.compactText)(usage.grouped
                            .slice(0, 6)
            ...
            ctx.Column({ padding: 12, spacing: 8 }, [
                ...
                rebuildJob
                    ? ctx.Column({ spacing: 6 }, [
                        ...
                        ctx.Text({
                            text: `${(0, format_1.formatPercent)(rebuildJob.fraction || 0)} · ${rebuildJob.status} · stage=${rebuildJob.currentStage}`,
                            style: "bodyMedium"
                        }),
                        ctx.Text({
