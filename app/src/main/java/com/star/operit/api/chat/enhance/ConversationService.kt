...
import com.star.operit.core.tools.UIPageResultData
import com.star.operit.core.tools.SimplifiedUINode
...
import com.star.operit.core.tools.ComputerDesktopActionResultData
...
import com.star.operit.api.chat.llmprovider.MediaLinkBuilder
    ...
    private suspend fun buildWaifuRulesText(): String {
        ...
        if (waifuEnableEmoticons) {
            ...
            val availableCategories = try {
                ...
                com.star.operit.util.AppLogger.e("ConversationService", "获取表情分组失败", e)
                emptyList()
            }
...
${FunctionalPrompts.translationUserPrompt(targetLanguage, text)}
    ...
    suspend fun analyzeImageWithIntent(
        ...
        return try {
            ...
            // 添加图片到池子并获取ID
            val imageId = com.star.operit.util.ImagePoolManager.addImage(imagePath)
            if (imageId == "error") {
            ...
            // 清理图片缓存
            com.star.operit.util.ImagePoolManager.removeImage(imageId)
    ...
    suspend fun analyzeAudioWithIntent(
        ...
        return try {
            ...
            val mediaId = com.star.operit.util.MediaPoolManager.addMedia(audioPath, mimeType)
            if (mediaId == "error") {
            ...
            com.star.operit.util.MediaPoolManager.removeMedia(mediaId)
            ChatUtils.removeThinkingContent(result.toString()).trim()
        } catch (e: Exception) {
    ...
    suspend fun analyzeVideoWithIntent(
        ...
        return try {
            ...
            val mediaId = com.star.operit.util.MediaPoolManager.addMedia(videoPath, mimeType)
            if (mediaId == "error") {
