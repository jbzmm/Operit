...
import com.star.operit.data.model.ToolValidationResult
...
class MCPManager(private val context: Context) {
    ...
    private val clientCache =
            ConcurrentHashMap<String, com.star.operit.data.mcp.plugins.MCPBridgeClient>()
    ...
    fun getOrCreateClient(
            serverName: String
    ): com.star.operit.data.mcp.plugins.MCPBridgeClient? {
        // 检查缓存中是否已有客户端
        ...
        try {
            // 创建新的桥接客户端
            val client =
                    com.star.operit.data.mcp.plugins.MCPBridgeClient(context, serverName)
