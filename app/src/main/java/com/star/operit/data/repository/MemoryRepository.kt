...
import com.star.operit.data.db.ObjectBoxManager
import com.star.operit.data.model.Memory
import com.star.operit.data.model.MemoryLink
import com.star.operit.data.model.MemoryTag
import com.star.operit.data.model.MemoryTag_
import com.star.operit.data.model.Memory_
import com.star.operit.data.model.DocumentChunk
import com.star.operit.data.model.Embedding
import com.star.operit.data.model.CloudEmbeddingConfig
import com.star.operit.data.model.DimensionCount
import com.star.operit.data.model.EmbeddingDimensionUsage
import com.star.operit.data.model.EmbeddingRebuildProgress
import com.star.operit.data.preferences.MemorySearchSettingsPreferences
import com.star.operit.services.CloudEmbeddingService
import com.star.operit.ui.features.memory.screens.graph.model.Edge
import com.star.operit.ui.features.memory.screens.graph.model.Graph
import com.star.operit.ui.features.memory.screens.graph.model.Node
...
import com.star.operit.data.model.MemoryExportData
import com.star.operit.data.model.SerializableMemory
import com.star.operit.data.model.SerializableLink
import com.star.operit.data.model.ImportStrategy
import com.star.operit.data.model.MemoryImportResult
import com.star.operit.data.model.MemorySearchDebugCandidate
import com.star.operit.data.model.MemorySearchDebugInfo
...
import com.star.operit.util.vector.IndexItem
import com.star.operit.util.vector.VectorIndexManager
...
class MemoryRepository(private val context: Context, profileId: String) {
    ...
    suspend fun deleteMemory(memoryId: Long): Boolean = withContext(Dispatchers.IO) {
        ...
        if (memory.isDocumentNode) {
            ...
            if (chunkIds.isNotEmpty()) {
                ...
                com.star.operit.util.AppLogger.d("MemoryRepo", "Deleted ${chunkIds.size} associated chunks for document.")
            }
        }
        ...
        if (linkIdsToDelete.isNotEmpty()) {
            ...
            com.star.operit.util.AppLogger.d(
                "MemoryRepo",
    ...
    private fun cleanupDanglingLinksIfNeeded(force: Boolean = false) {
        ...
        if (danglingLinkIds.isNotEmpty()) {
            ...
            com.star.operit.util.AppLogger.w(
                "MemoryRepo",
    ...
    suspend fun linkMemories(
        ...
        if (existingLink != null) {
            // 链接已存在，可以选择更新或直接返回
            // 这里我们选择直接返回，不创建重复链接
            com.star.operit.util.AppLogger.d("MemoryRepo", "Link already exists from memory ${source.id} to ${target.id} with type $type")
            return@withContext
        }
    ...
    private suspend fun runSearchMemoriesWithDebug(
        ...
        if (memoriesToSearch.isEmpty()) {
            com.star.operit.util.AppLogger.d("MemoryRepo", "No memories found in folder '$folderPath' to search.")
            return@withContext SearchComputationResult(
                memories = emptyList(),
                debug = buildDebug(
        ...
        if (reverseContainmentResults.isNotEmpty()) {
            com.star.operit.util.AppLogger.d("MemoryRepo", "Reverse containment: ${reverseContainmentResults.size} matches")
        }
        if (effectiveKeywordWeight > 0.0) {
        ...
        if (effectiveSemanticWeight > 0.0f && cloudConfig.isReady()) {
            com.star.operit.util.AppLogger.d("MemoryRepo", "--- Starting Semantic Search for ${keywords.size} keywords ---")
            keywords.forEach { keyword ->
                val queryEmbedding = generateEmbedding(keyword, cloudConfig)
                if (queryEmbedding == null) {
                    com.star.operit.util.AppLogger.w("MemoryRepo", "Failed to generate embedding for: '$keyword'")
                    return@forEach
                }
            ...
            com.star.operit.util.AppLogger.d("MemoryRepo", "--- Semantic Search Completed ---")
        }
        ...
        if (edgesTraversed > 0) {
            com.star.operit.util.AppLogger.d("MemoryRepo", "Graph expansion: ${edgesTraversed} edges traversed")
        }
        ...
        com.star.operit.util.AppLogger.d("MemoryRepo", "Final results: ${filteredScores.size}/${scores.size} above threshold")
        ...
        sortedScoresForLogging.take(3).forEach { (id, score) ->
            ...
            if (memory != null) {
                com.star.operit.util.AppLogger.d("MemoryRepo", "  Top: [${memory.title}] = ${String.format("%.4f", score)}")
            }
        }
        ...
        if (filteredScores.isEmpty()) {
            com.star.operit.util.AppLogger.d("MemoryRepo", "No memories above relevance threshold")
            return@withContext SearchComputationResult(
                memories = emptyList(),
                debug = debugInfo
            )
    ...
    suspend fun searchChunksInDocument(memoryId: Long, query: String, limit: Int = 20): List<DocumentChunk> = withContext(Dispatchers.IO) {
        com.star.operit.util.AppLogger.d("MemoryRepo", "--- Starting search in document (Memory ID: $memoryId) for query: '$query' ---")
        val memory = findMemoryById(memoryId) ?: return@withContext emptyList<DocumentChunk>().also {
            com.star.operit.util.AppLogger.w("MemoryRepo", "Document with ID $memoryId not found.")
        }
        if (!memory.isDocumentNode) {
        ...
        if (query.isBlank()) {
            com.star.operit.util.AppLogger.d("MemoryRepo", "Query is blank, returning up to $validLimit chunks sorted by index.")
        ...
        if (effectiveSemanticWeight > 0.0f && cloudConfig.isReady()) {
            keywords.forEach { keyword ->
                val queryEmbedding = generateEmbedding(keyword, cloudConfig)
                if (queryEmbedding == null) {
                    com.star.operit.util.AppLogger.w("MemoryRepo", "Failed to generate chunk embedding query for: '$keyword'")
                    return@forEach
                }
        ...
        if (scores.isEmpty()) {
            com.star.operit.util.AppLogger.d("MemoryRepo", "No matching chunks found after weighted ranking.")
            return@withContext emptyList()
        }
        ...
        val combinedResults = scores.entries
            ...
            .take(validLimit)
        com.star.operit.util.AppLogger.d("MemoryRepo", "Weighted document chunk results count: ${combinedResults.size}. Limit=$validLimit.")
        com.star.operit.util.AppLogger.d("MemoryRepo", "--- Search in document finished ---")
    ...
    suspend fun getAllFolderPaths(): List<String> = withContext(Dispatchers.IO) {
        ...
        com.star.operit.util.AppLogger.d("MemoryRepository", "getAllFolderPaths: Total memories: ${allMemories.size}")
        val folderPaths = allMemories
            .map { normalizeFolderPath(it.folderPath) ?: context.getString(R.string.memory_uncategorized) }
            .distinct()
            .sorted()
        com.star.operit.util.AppLogger.d("MemoryRepository", "getAllFolderPaths: Unique folders: $folderPaths")
        folderPaths
    }
    ...
    suspend fun renameFolder(oldPath: String, newPath: String): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            com.star.operit.util.AppLogger.e("MemoryRepo", "Failed to rename folder", e)
            false
        }
    }
    ...
    suspend fun moveMemoriesToFolder(memoryIds: List<Long>, targetFolderPath: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            com.star.operit.util.AppLogger.e("MemoryRepo", "Failed to move memories", e)
            false
        }
    }
    ...
    suspend fun createFolder(folderPath: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            com.star.operit.util.AppLogger.e("MemoryRepo", "Failed to create folder", e)
            false
        }
    }
    ...
    suspend fun mergeMemories(
        ...
        if (sourceMemories.size < 2) {
            com.star.operit.util.AppLogger.w("MemoryRepo", "Merge requires at least two unique source memories to be found. Found: ${sourceMemories.size} from titles: ${sourceTitles.joinToString()}.")
        ...
        try {
            ...
            com.star.operit.util.AppLogger.e("MemoryRepo", "Error during memory merge transaction.", e)
            return@withContext null
        }
    ...
    suspend fun deleteMemoriesByUuids(uuids: Set<String>): Boolean = withContext(Dispatchers.IO) {
        com.star.operit.util.AppLogger.d("MemoryRepo", "Attempting to delete memories with UUIDs: $uuids")
        if (uuids.isEmpty()) {
            com.star.operit.util.AppLogger.d("MemoryRepo", "UUID set is empty, nothing to delete.")
        ...
        com.star.operit.util.AppLogger.d("MemoryRepo", "Found ${memoriesToDelete.size} memories to delete.")
        ...
        try {
            store.runInTx {
                ...
                com.star.operit.util.AppLogger.d("MemoryRepo", "Found ${linkIdsToDelete.size} unique links and ${chunkIdsToDelete.size} chunks to delete.")

                // 2. 批量删除链接和区块
                if (linkIdsToDelete.isNotEmpty()) {
                    linkBox.removeByIds(linkIdsToDelete)
                    com.star.operit.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${linkIdsToDelete.size} links.")
                }
                if (chunkIdsToDelete.isNotEmpty()) {
                    chunkBox.removeByIds(chunkIdsToDelete)
                    com.star.operit.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${chunkIdsToDelete.size} chunks.")
                }

                // 3. 批量删除记忆本身
                memoryBox.removeByIds(memoryIdsToDelete.toList())
                com.star.operit.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${memoriesToDelete.size} memories.")
            }
            com.star.operit.util.AppLogger.d("MemoryRepo", "Transaction completed successfully.")
        } catch (e: Exception) {
            com.star.operit.util.AppLogger.e("MemoryRepo", "Error during bulk delete transaction.", e)
            return@withContext false
        }

        // 4. 在事务外处理向量索引和文件
        memoriesToDelete.forEach(::removeMemoryFromIndexInternal)
        com.star.operit.util.AppLogger.d("MemoryRepo", "Removed deleted memories from vector indices and cleaned up chunk index files.")
        ...
        memories.forEach { memory ->
            ...
            memory.links.forEach { link ->
                ...
                if (sourceId != null &&
                    ...
                    // com.star.operit.util.AppLogger.d("MemoryRepo", "Discarding edge: $sourceId -> $targetId
    ...
    suspend fun deleteFolder(folderPath: String) {
        withContext(Dispatchers.IO) {
            ...
            com.star.operit.util.AppLogger.d("MemoryRepo", "Deleted folder '$folderPath', moved ${memories.size} memories to uncategorized")
        }
    }
    ...
    suspend fun importMemoriesFromJson(
        ...
        try {
            ...
            com.star.operit.util.AppLogger.d("MemoryRepo", "Import completed: $newCount new, $updatedCount updated, $skippedCount skipped, $newLinksCount links")
            ...
            com.star.operit.util.AppLogger.e("MemoryRepo", "Failed to import memories", e)
            throw e
        }
    }
